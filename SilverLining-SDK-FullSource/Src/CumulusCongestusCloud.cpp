...
void CumulusCongestusCloud::InitializeVoxelState(const RandomNumberGenerator* rng, ThreadCameraStreamData* tcsData)
{
    voxelsDirty = true;
        ...
        for (j = 0; j < depth; j++) {
            for (k = 0; k < height; k++) {
                ...
                if (voxelSize > 2.0) {
                    ...
                    pos.y += rng->UniformRandomIntRange(0, (int)(voxelSize * 0.5) - 1);
...
void CumulusCongestusCloud::ApplyWindShear(const Vector3& wind, ThreadCameraStreamData* tcsData)
{
    double updraft = 10.0; // m/s
    Configuration::GetDoubleValue("cumulus-congestus-updraft-speed", updraft);
    updraft *= Atmosphere::GetUnitScale();
    CumulusCloud::Shear(updraft, wind, tcsData);
}

void CumulusCongestusCloud::IncrementTimeStep(unsigned long now, const RandomNumberGenerator* rng, const BillboardBufferProvider* provider, ThreadCameraStreamData* tcsData)
{
    voxelsDirty = true;
