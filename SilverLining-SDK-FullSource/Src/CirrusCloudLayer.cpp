...
CirrusCloudLayer::CirrusCloudLayer(const Atmosphere& atmosphere) : CloudLayer(atmosphere)
{
...
bool CirrusCloudLayer::DrawSetup(int pass, const Vector3 *lightPos, const Color *lightColor, ThreadCameraStreamData* tcsData) const
{
    return true;
}

bool CirrusCloudLayer::EndDraw(int pass) const
{
    return true;
}

bool CirrusCloudLayer::SeedClouds(const Atmosphere& atm, void* data)
{
    ...
    ClearClouds(tcsData);
    CirrusCloud *cloud = SL_NEW CirrusCloud(this, atmosphere.GetRandomNumberGenerator()->UniformRandomDouble());
    double x, z;
    ...
    cloud->Init(w, h, atmosphere.GetRandomNumberGenerator(), tcsData);
...
void CirrusCloudLayer::WrapClouds(bool geocentric, const Camera* camera, CloudLayerTcsUserData* tcsUserData)
{
...
void CirrusCloudLayer::ProcessAtmosphericEffects(Sky *sky, const Atmosphere* atmosphere, const Camera* camera, ThreadCameraStreamData* tcsData) const
{
...
bool CirrusCloudLayer::RestoreClouds(const Atmosphere& atm, std::istream& s, ThreadCameraStreamData* tcsData)
{
    int nClouds;
...
ShaderHandle CirrusCloudLayer::GetShader(ThreadCameraStreamData* tcsData) const
{
    return tcsData->GetCirrusShader()->GetShader();
}

void CirrusCloudLayer::ReloadShaders(ThreadCameraStreamData* tcsData)
{
