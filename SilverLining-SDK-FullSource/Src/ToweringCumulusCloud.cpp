...
ToweringCumulusCloud::ToweringCumulusCloud(CloudLayer *layer, double coverageThreshold, bool voxelsUseShaderInstancing, bool _drawIndirect, ThreadCameraStreamData* _tcsData) : CumulusCloud(layer, coverageThreshold, voxelsUseShaderInstancing, _drawIndirect, _tcsData)
{
    boundScale = 0.5;
    flattening = 1.0f;
    Configuration::GetFloatValue("towering-cumulus-voxel-flattening", flattening);
}

void ToweringCumulusCloud::InitializeVoxelState(const RandomNumberGenerator* rng, ThreadCameraStreamData* tcsData)
{
    voxelsDirty = true;
...
void ToweringCumulusCloud::ApplyWindShear(const Vector3& wind, ThreadCameraStreamData* tcsData)
{
...
void ToweringCumulusCloud::IncrementTimeStep(unsigned long now, const RandomNumberGenerator* rng, const BillboardBufferProvider* provider, ThreadCameraStreamData* tcsData)
{
    voxelsDirty = true;
