...
CirrusCloud::CirrusCloud(CloudLayer *layer, double coverageThreshold)
...
void CirrusCloud::Init(double w, double h, const RandomNumberGenerator* rng, ThreadCameraStreamData* _tcsData)
{
    tcsData = _tcsData;
    ...
    if (hires) {
        texture = textureManager->GetOrCreateCirrusHiResTexture();
    } else {
        int whichCirrus = rng->UniformRandomIntRange(0, 1);
        texture = textureManager->GetOrCreateCirrusTexture(whichCirrus);
    }
    ...
    name = (enableObjectLabeling)? "CirrusCloud::VertexBuffer" : (const char *)0;
    ...
    name = (enableObjectLabeling)? "CirrusCloud::IndexBuffer" : (const char *)0;
...
bool CirrusCloud::IsCulled(const Camera* cullCamera, ThreadCameraStreamData* tcsData) const
{
    return false;
}

bool CirrusCloud::Draw(int pass, const Vector3 &lightPos, const Vector3& lightDir,
                       const Color &lightColor, bool invalid, const Sky *sky, bool, const Camera* camera
...
void CirrusCloud::DrawBlendedObject(const Atmosphere* atmosphere, const Camera* sceneCamera, const Camera* renderCamera, const OverriddenBillboardMatrix& overriddenBillboardMatrix, const OverriddenCameraPos& overriddenCameraPos, bool fadeOverride
                                    , ThreadCameraStreamData* tcsData) const
{
    ...
    if (parentCloudLayer->GetIsInfinite()) {
        ...
        Matrix4 orientation = renderCamera->GetModelViewMatrix();
        ...
        adjustedCam.MultiplyModelViewMatrix(xlate*sceneCamera->GetInverseBasis3x3());
        renderer->SetModelviewMatrix(adjustedCam.GetModelViewMatrix());
    }
...
bool CirrusCloud::Update(unsigned long now, bool forceUpdate, const RandomNumberGenerator* rng, const BillboardBufferProvider* provider, ThreadCameraStreamData* tcsData)
{
    return false;
}

Vector3 CirrusCloud::GetSortPosition(const Camera* camera, const ThreadCameraStreamData* tcsData) const
{
    double x, y, z;
...
double CirrusCloud::GetDistance(const Vector3& from, const Vector3& to, const Matrix4& mv, bool rightHanded, const Camera* camera, ThreadCameraStreamData* tcsData) const
{
