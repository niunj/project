...
CirroCumulusCloud::CirroCumulusCloud(CloudLayer *layer, double coverageThreshold)
...
void CirroCumulusCloud::Init(double w, double h, ThreadCameraStreamData* _tcsData)
{
    tcsData = _tcsData;
    width = w;
    height = h;
    ...
    TextureManager* textureManager = (GetParentCloudLayer()->GetAtmosphere().TexturesAreShared()) ? GetParentCloudLayer()->GetAtmosphere().GetDefaultTcsData()->GetTextureManager() : tcsData->GetTextureManager();
    texture = textureManager->GetOrCreateCirrocumulusTexture();
    ...
    name = (enableObjectLabeling)? "CirroCumulusCloud::VertexBuffer" : (const char *)0;
    ...
    name = (enableObjectLabeling)? "CirroCumulusCloud::IndexBuffer" : (const char *)0;
...
CirroCumulusCloud::~CirroCumulusCloud()
{
    Renderer* renderer = tcsData->GetRenderer();
...
bool CirroCumulusCloud::IsCulled(const Camera* cullCamera, ThreadCameraStreamData* tcsData) const
{
    return false;
}

bool CirroCumulusCloud::Draw(int pass, const Vector3 &lightPos, const Vector3& lightDir
...
void CirroCumulusCloud::DrawBlendedObject(const Atmosphere* atmosphere, const Camera* sceneCamera, const Camera* renderCamera, const OverriddenBillboardMatrix& overriddenBillboardMatrix, const OverriddenCameraPos& overriddenCameraPos, bool fadeOverride
        , ThreadCameraStreamData* tcsData) const
{
    ...
    renderer->IncrementConstantBuffersOffset(hdrShader);
...
bool CirroCumulusCloud::Update(unsigned long now, bool forceUpdate, const RandomNumberGenerator* rng, const BillboardBufferProvider* provider, ThreadCameraStreamData* tcsData)
{
    return false;
}

Vector3 CirroCumulusCloud::GetSortPosition(const Camera* camera, const ThreadCameraStreamData* tcsData) const
{
    double x, y, z;
...
double CirroCumulusCloud::GetDistance(const Vector3& from, const Vector3& to, const Matrix4& mv, bool rightHanded, const Camera* camera, ThreadCameraStreamData* tcsData) const
{
...
void CirroCumulusCloud::GetSize(double& pwidth, double& pdepth, double& pheight) const
{
    pwidth = width;
    pdepth = height;
