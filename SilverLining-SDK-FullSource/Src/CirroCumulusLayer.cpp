...
CirroCumulusCloudLayer::CirroCumulusCloudLayer(const Atmosphere& atmosphere) : CloudLayer(atmosphere)
{
...
bool CirroCumulusCloudLayer::DrawSetup(int pass, const Vector3 *lightPos, const Color *lightColor, ThreadCameraStreamData* tcsData) const
{
    return true;
}

bool CirroCumulusCloudLayer::EndDraw(int pass) const
{
    return true;
}

bool CirroCumulusCloudLayer::SeedClouds(const Atmosphere& atm, void* data)
{
    ...
    ClearClouds(tcsData);
    CirroCumulusCloud *cloud = SL_NEW CirroCumulusCloud(this, atmosphere.GetRandomNumberGenerator()->UniformRandomDouble());
    double x, z;
...
void CirroCumulusCloudLayer::ProcessAtmosphericEffects(Sky *sky, const Atmosphere* atmosphere, const Camera* camera, ThreadCameraStreamData* tcsData) const
{
...
void CirroCumulusCloudLayer::WrapClouds(bool geocentric, const Camera* camera, CloudLayerTcsUserData* tcsUserData)
{
...
bool CirroCumulusCloudLayer::RestoreClouds(const Atmosphere& atm, std::istream& s, ThreadCameraStreamData* tcsData)
{
    int nClouds;
...
ShaderHandle CirroCumulusCloudLayer::GetShader(ThreadCameraStreamData* tcsData) const
{
    return tcsData->GetCirrusShader()->GetShader();
}

void CirroCumulusCloudLayer::ReloadShaders(ThreadCameraStreamData* tcsData)
{
