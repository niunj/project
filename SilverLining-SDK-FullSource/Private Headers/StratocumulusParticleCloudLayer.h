...
/**
\file StratocumulusParticleCloudLayer.h
\brief A CloudLayer that contains many individual low, puffy clouds that together make for a thick
cloud layer with a few sun breaks. This representation is a thin layer of stretched-out particles.
*/

#ifndef STRATOCUMULUS_PARTICLE_CLOUD_LAYER_H
#define STRATOCUMULUS_PARTICLE_CLOUD_LAYER_H
...
namespace SilverLining
{
    ...
    class StratocumulusParticleCloudLayerTcsUserData;

    /** A layer of thin cloud particles following an advective noise pattern. */
    class StratocumulusParticleCloudLayer : public CloudLayer, public Renderable
    {
    public:
        /** Default constructor. */
        StratocumulusParticleCloudLayer(const Atmosphere& atmosphere);

        /** Virtual destructor. */
        virtual ~StratocumulusParticleCloudLayer();
        ...
        */
        ...
        static int GetRecompileFrameCount() { return recompileFrameCount; }
        static void IncrementRecompileFrameCount() { recompileFrameCount++; }
    ...
    protected:
        ...
        virtual void RoundEdgesAndApplyDensity(const Camera* camera, const Atmosphere *atm, CloudLayerTcsUserData* tcsUserData);
        double GetWrapFade(StratocumulusParticleCloudQuad *q, const Vector3& anchor, bool fadeOverride, CloudLayerTcsUserData* tcsUserData) const;
        StratocumulusParticleCloudLayerTcsUserData* GetOrCreateSpcTcsUserData(ThreadCameraStreamData* data) const;

        void MakeQuad(int x, int y, int quadSize, TGALoader& tga, const Atmosphere& atm, ThreadCameraStreamData* tcsData);

		double DistanceToPlane(const Vector3& from, const Vector3& to, double D, const Matrix4& mv, ThreadCameraStreamData* tcsData) const;

        float voxelSize;
        int voxelsWidth, voxelsDepth;

        float gradient;

        double thicknessVariance;

        double cosLightThreshold;
        
        float ambientBlend, diffuseScale;
        ...
        static int recompileFrameCount;
    };
}
