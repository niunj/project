...
/** \file CirroCumulusCloud.h
   \brief A class that knows how to draw cirrostratus cloud decks.
 */
...
/** A class responsible for the rendering of a CirroStratus cloud. */
class CirroStratusCloud : public Cloud
{
public:
/** Default constructor. */
    CirroStratusCloud(CloudLayer *layer, double coverageThreshold);

/** The destructor deletes any vertex and index buffer objects required to
   draw this cloud. */
    virtual ~CirroStratusCloud();
...
/** Prior to drawing this cloud, it must know how big it is so it can create the
   ...
   required to draw a cirrostratus cloud of a specified size. Note that cirrostratus clouds are
   drawn as a 2D plane, they have no thickness, and so this method does not accept
   a thickness parameter.

   \param w The width of the cirrostratus cloud in world units.
   \param h The depth of the cirrostratus cloud in world units.
 */
    virtual void Init(double w, double h, ThreadCameraStreamData* tcsData);

/** Tests if this cirrostratus cloud deck is visible within the specified frustum.
   ...
   culled. cirrostratus decks are large enough and simple enough to draw that this just returns
...
/** Draws this cirrostratus cloud under the given lighting conditions. In fact, drawing may be
...
/** cirrostratus clouds do not model growth over time, and so its Update() method is an
...
/** Returns the dimensions of this cirrostratus cloud. */
    virtual void GetSize(double& width, double& depth, double& height) const;

/** cirrostratus clouds do not use dynamically-generated imposters, and so CreateImposter() does
