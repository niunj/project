...
int main(int argc, char** argv)
{
    try {
        ...
        windowTraits->deviceExtensionNames.push_back(VK_EXT_DESCRIPTOR_INDEXING_EXTENSION_NAME);

        auto features = windowTraits->deviceFeatures = vsg::DeviceFeatures::create();
        auto& descriptorIndexingFeatures = features->get<VkPhysicalDeviceDescriptorIndexingFeaturesEXT, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DESCRIPTOR_INDEXING_FEATURES_EXT>();
        ...
        window->clearColor() = VkClearColorValue{{0.0f, 0.0f, 0.2f, 1.0f}};
        ...
        auto silverLiningGroup = SilverLiningGroup::create(window);
        auto silverLiningCommand = SilverLiningCommand::create(silverLiningGroup->atmosphere(), silverLiningGroup);
        silverLiningGroup->addChild(silverLiningCommand);

        vsg_scene->addChild(silverLiningGroup);

        if (!group->children.empty()) {
            if (group->children.size() == 1)
                vsg_scene->addChild(group->children[0]);
            else
        ...
        if (ellipsoidModel) {
            ...
            perspective = vsg::Perspective::create(60.0, static_cast<double>(window->extent2D().width) / static_cast<double>(window->extent2D().height), nearFarRatio * radius, 100000);
        }
