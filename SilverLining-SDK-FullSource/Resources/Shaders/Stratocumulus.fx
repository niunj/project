...
#define SAMPLE_TEX3D( texture, sampler, coord ) tex3Dlod( sampler, float4(coord, 0.0) )
...
float2 getCloudDensity(in float3 texCoord, in float t)
{
    ...
    for( int o = 0; o < NOISE_OCTAVES; o++ ) {
        uvw.xyz += uvw.xyz;
        perturb.xyz += ( SAMPLE_TEX3D( gDiffuseMap2, gNoiseSampler, uvw.xyz ).xyz - 0.5 ) * scale;
        scale *= 0.5;
    }
#else
#if ( NOISE_OCTAVES > 0 )
    perturb.xyz += 1.0  * SAMPLE_TEX3D( gDiffuseMap2, gNoiseSampler, 2.0 * uvw.xyz).xyz - 0.5;
#endif
#if ( NOISE_OCTAVES > 1 )
    perturb.xyz += 0.5  * SAMPLE_TEX3D( gDiffuseMap2, gNoiseSampler, 4.0 * uvw.xyz).xyz - 0.25;
#endif
#if ( NOISE_OCTAVES > 2 )
    perturb.xyz += 0.25 * SAMPLE_TEX3D( gDiffuseMap2, gNoiseSampler, 8.0 * uvw.xyz).xyz - 0.125;
#endif
#if ( NOISE_OCTAVES > 3 )
    perturb.xyz += 0.125 * SAMPLE_TEX3D( gDiffuseMap2, gNoiseSampler, 16.0 * uvw.xyz).xyz - 0.0625;
#endif
#if ( NOISE_OCTAVES > 4 )
    perturb.xyz += 0.0625 * SAMPLE_TEX3D( gDiffuseMap2, gNoiseSampler, 32.0 * uvw.xyz).xyz - 0.03125;
#endif
#endif
    ...
    return SAMPLE_TEX3D(gDiffuseMap, gCloudSampler, sampleCoord.xzy).xy;
