...
float4 xyYtoRGB(float3 xyY, float4x4 XYZtoRGB, float oneOverGamma)
{
    ...
    RGB = pow(abs(RGB), oneOverGamma); // apply gamma correction
         ...
         )
{
    ...
    xyY = toneMap(xyY, sl_luminanceScales.y, sl_luminanceScales.x, sl_kAndLdmax.x,
                  sl_kAndLdmax.y);
    ...
    float4 skyColor = xyYtoRGB(xyY, sl_XYZtoRGB, sl_kAndLdmax.z);
