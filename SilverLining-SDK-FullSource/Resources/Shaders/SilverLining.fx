...
float4 PS(SL_Vertex inVert ) : SV_TARGET {
    ...
    if (gTexture2D)
    {
        if (gTextureRepeatU && gTextureRepeatV) {
            fragIn = gDiffuseMap.Sample(gTriLinearSamWrapWrap, inVert.tex.xy);
        } else if (gTextureRepeatU && !gTextureRepeatV) {
            fragIn = gDiffuseMap.Sample(gTriLinearSamWrapClamp, inVert.tex.xy);
        } else if (!gTextureRepeatU && gTextureRepeatV) {
            fragIn = gDiffuseMap.Sample(gTriLinearSamClampWrap, inVert.tex.xy);
        } else if (!gTextureRepeatU && !gTextureRepeatV) {
            fragIn = gDiffuseMap.Sample(gTriLinearSamClampClamp, inVert.tex.xy);
        }
    }
...
void WideLineGeometryShader(line SL_Vertex input[2], inout TriangleStream<SL_Vertex> WideLineStream)
{
    ...
    float4 x = float4(gModelview[0][0], gModelview[1][0], gModelview[2][0], 1.0);
