...
void MyMessageBox::setText(const QString& text) {
	//m_pTextLabel->setFont(QFont(TEXTFONTFAMILY,TEXTFONTSIZE,75));
	m_pTextLabel->setStyleSheet("font:bold 25px;");
...
// 自定义消息框：显示“是”“否”按钮
bool MyMessageBox::showCustomMessageBox(QString title, QString content,QString sureButton,QString calcelButton) {
    // 创建消息框（警告类型，标题和内容自定义）
    QMessageBox msgBox(QMessageBox::Warning,
                      title,
                      content,
                      QMessageBox::NoButton,  // 先不添加任何默认按钮
                      qApp->activeWindow());  // 父窗口

    // 手动添加“是”按钮（设置为AcceptRole：确认操作）
    QPushButton* yesBtn = new QPushButton(sureButton);
    msgBox.addButton(yesBtn, QMessageBox::AcceptRole);

    // 手动添加“否”按钮（设置为RejectRole：取消操作）
    QPushButton* noBtn = new QPushButton(calcelButton);
    msgBox.addButton(noBtn, QMessageBox::RejectRole);
